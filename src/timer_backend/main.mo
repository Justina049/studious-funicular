...
actor SimpleTimer {
  ...
  public func pause() {
    ...
    if (isRunning) {
      ...
      isRunning := false;
      Debug.print("⏸️ Timer paused. Total time: " # Int.toText(totalTime) # " seconds");
    } else {
      Debug.print("⚠️ Timer is not running.");
    }
  };
  ...
  public func resume() {
    ...
    if (not isRunning) {
      startTime := Time.now() / 1_000_000_000;
      isRunning := true;
      Debug.print("▶️ Timer resumed at: " # Int.toText(startTime) # " seconds");
    } else {
      Debug.print("⚠️ Timer is already running.");
    }
  };
  ...
  public func stop() : async Int {
    ...
    totalTime := 0;
    Debug.print("⏹️ Timer stopped. Total time: " # Int.toText(finalTime) # " seconds");
    return finalTime;
  };
